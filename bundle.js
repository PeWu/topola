...
var AncestorChart = /** @class */ (function () {
     ...
     *     be drawn.
     */
    function AncestorChart(data, renderer, startId) {
        this.data = data;
    ...
    AncestorChart.prototype.render = function () {
        ...
        d3.select('svg g').attr('transform', "translate(" + (-y0 + MARGIN) + ", " + (-x0 + MARGIN) + ")");
        // Render nodes.
        var nodeEnter = d3.select('svg g')
            .selectAll('g.node')
...
function render(options) {
    d3.json(options.jsonUrl).then(function (json) {
        ...
        var indiUrlFunction = options.indiUrl ?
            ...
            undefined;
        var renderer = new topola_render_1.SimpleRenderer(data, indiUrlFunction);
        var chart = new topola_chart_1.AncestorChart(data, renderer, options.startId);
        chart.render();
    });
}
